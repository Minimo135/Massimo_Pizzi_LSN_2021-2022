...
int main(int argc, char *argv[]){
  ...
  for(int step=1; step <= nstep_eq; step++) //Simulation
  {
      Move();
      Measure();
      PrePrint(step);   //Print results for the preliminary equilibrization
  }
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Pres.open("output_ptot.dat", ios::app);
...
void PrePrint(int step){
  ofstream PreTemp;
   const int wd=12;
    
    if (step%100 == 0 ) {
      cout << "Equilibrization - step " << step  << endl;
      cout << "----------------------------" << endl << endl;
    }
    
    PreTemp.open("output_pretemp.dat",ios::app);

    stima_temp = walker[it]; //Temperature

// Struttura dell'output
//                    # step                    Valore             

  //Temperature print
  PreTemp << setw(wd) << step <<  setw(wd) << stima_temp << endl;
