...
chromosome & chromosome::mutate(){
  ...
  int np = 4;
  double perm[np] {0.05, 0.02, 0.02, 0.12};
...
vector<chromosome> Population::select(bool torunament){
  ...
  if(torunament){
    ...
    //Vengono selezionati due sottogruppi casuali di cromosomi da cui verranno estratti i due migliori come genitori
    //La dimensione del sottogruppo è casualmente posta tra l'8% e il 12% della dimenisione della popolazione
    int group_dim1 = ceil(m_dim_pop*rnd.Rannyu(0.08, 0.12));
    int group_dim2 = ceil(m_dim_pop*rnd.Rannyu(0.08, 0.12));
    ...
    //Genero un numero casuale compreso tra 0 e m_dim_pop - gorup_dim: ottengo il punto di inizio del gruppo
    int stp1 = int(rnd.Rannyu()*(m_dim_pop - group_dim1));
    int stp2 = int(rnd.Rannyu()*(m_dim_pop - group_dim2));
    //Inserisco i cromosomi dalla posizione stp in un array per gruppo
    ...
    //Ordino il gruppo ponendo all'inizio i migliori e li inserisco nel vettore di output
...
double estimate_fit(int * get_city, int size, vector<city> m_cities_list){
  ...
  for(int i = 1; i < size; i++){
    int label = get_city[i];
    int label2 = get_city[i-1];
    fit += pow(m_cities_list.at(label).x - m_cities_list.at(label2).x, 2) + pow(m_cities_list.at(label).y - m_cities_list.at(label2).y, 2);
  }
  int label = get_city[size-1];
  fit += pow(m_cities_list.at(0).x - m_cities_list.at(label).x, 2) + pow(m_cities_list.at(0).y - m_cities_list.at(label).y, 2);
